...
class AuditEventControllerTest {
    ...
    void postCreatesEventReturns201WithLocation() throws Exception {
        ...
        AuditEvent saved = entity(id, ts, "alice", "user.login", "session:1",
                Outcome.SUCCESS, "{\"ip\":\"1.2.3.4\"}");
        when(service.create("alice", "user.login", "session:1", Outcome.SUCCESS,
                "{\"ip\":\"1.2.3.4\"}")).thenReturn(saved);
    ...
    void getByActorOnly() throws Exception {
        AuditEvent event = entity(UUID.randomUUID(), Instant.parse("2026-04-25T10:00:00Z"),
                "alice", "x", "r", Outcome.SUCCESS, null);
        when(service.search(eq("alice"), isNull(), isNull(), isNull()))
                .thenReturn(List.of(event));

        mockMvc.perform(get("/audit-events").param("actor", "alice"))
    ...
    void getByResourceOnly() throws Exception {
        AuditEvent event = entity(UUID.randomUUID(), Instant.now(),
                "alice", "x", "project:42", Outcome.DENIED, null);
        when(service.search(isNull(), eq("project:42"), isNull(), isNull()))
                .thenReturn(List.of(event));

        mockMvc.perform(get("/audit-events").param("resource", "project:42"))
                .andExpect(status().isOk())
    ...
    void getByAllParamsCombined() throws Exception {
        ...
        AuditEvent event = entity(UUID.randomUUID(), Instant.parse("2026-03-04T12:00:00Z"),
                "mark.smith", "payment.charge", "payment-service", Outcome.SUCCESS, null);
        when(service.search("mark.smith", "payment-service", from, to))
                .thenReturn(List.of(event));
    ...
    void getMapsServiceIllegalArgumentTo400() throws Exception {
        ...
        mockMvc.perform(get("/audit-events"))
                ...
                .andExpect(jsonPath("$.message",
                        containsString("at least one filter")));
    }

    private static AuditEvent entity(UUID id, Instant ts, String actor, String action,
                                     String resource, Outcome outcome, String context) {
