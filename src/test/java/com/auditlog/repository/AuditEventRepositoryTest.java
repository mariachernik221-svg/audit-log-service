...
class AuditEventRepositoryTest {
    ...
    void persistsAndReadsBackById() {
        AuditEvent saved = repository.save(
                new AuditEvent("alice", "user.login", "session:1", Outcome.SUCCESS, "{\"ip\":\"1.2.3.4\"}"));
    ...
    void searchByActorOnlyReturnsOrderedNewestFirst() {
        AuditEvent older = save("alice", "x.y", "r:1", Outcome.SUCCESS, Instant.now().minus(2, ChronoUnit.HOURS));
        AuditEvent newer = save("alice", "x.y", "r:2", Outcome.SUCCESS, Instant.now().minus(1, ChronoUnit.HOURS));
        save("bob", "x.y", "r:3", Outcome.SUCCESS, Instant.now());

        List<AuditEvent> result = repository.search("alice", null, null, null);

        assertThat(result).extracting(AuditEvent::getId)
    ...
    void searchByResourceOnly() {
        ...
        AuditEvent target = save("bob", "x.y", "project:42", Outcome.DENIED, Instant.now().minus(1, ChronoUnit.HOURS));
    ...
    void searchByTimeRangeOnlyHalfOpen() {
        ...
        assertThat(result).extracting(AuditEvent::getId)
                .containsExactly(inB.getId(), inA.getId());
    }
    ...
    void searchByAllFiltersCombined() {
        ...
        AuditEvent target = save("mark.smith", "payment.charge", "payment-service",
                Outcome.SUCCESS, Instant.parse("2026-03-04T12:00:00Z"));
        save("mark.smith", "payment.charge", "payment-service",
                Outcome.SUCCESS, t0.minusSeconds(1));
        save("mark.smith", "payment.charge", "payment-service",
                Outcome.SUCCESS, t1.plusSeconds(1));
        save("mark.smith", "payment.charge", "billing-service",
                Outcome.SUCCESS, Instant.parse("2026-03-04T12:00:00Z"));
        save("alice", "payment.charge", "payment-service",
                Outcome.SUCCESS, Instant.parse("2026-03-04T12:00:00Z"));
    ...
    private AuditEvent save(String actor, String action, String resource, Outcome outcome, Instant ts) {
